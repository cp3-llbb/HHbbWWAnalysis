...
class NonClosureFake:
    ...
    def additional(self,h,key,systName,**kwargs):
        ...
        lambda_nom_up     = lambda x : 1 + abs(1-nom)
        lambda_nom_down   = lambda x : 1 - abs(1-nom)
        lambda_slope_up   = lambda x : 1 + max(min(slope*x,1.),-1.)
        lambda_slope_down = lambda x : 1 - max(min(slope*x,1.),-1.)
