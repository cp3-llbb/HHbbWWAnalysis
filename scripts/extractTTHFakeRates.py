...
for cat,h_nom,h_ups,h_downs in zip(["Electron","Muon"],[el_nom,mu_nom],[el_hist_up,mu_hist_up],[el_hist_down,mu_hist_down]):
    ...
    for y in range(1,h_nom.GetNbinsY()+1):
        ...
        for x in range(1,h_nom.GetNbinsX()+1):
            ...
            err_up = 0.
            err_down = 0.
            for h_up,h_down in zip(h_ups,h_downs):
                err_up += h_up.GetBinContent(x,y)**2
                err_down += h_down.GetBinContent(x,y)**2
            ydict['values'].append({'bin':[xMin,xMax],
                                    'value':nom,
                                    'error_low':math.sqrt(err_down),
                                    'error_high':math.sqrt(err_up)})
        data.append(ydict)
    ...
    filename = 'TTHFakeRates_%s_%s.json'%(cat,args.era)
